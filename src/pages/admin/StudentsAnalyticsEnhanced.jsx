...
export default function StudentsAnalyticsEnhanced() {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (skillsGapChartInstance.current) skillsGapChartInstance.current.destroy();
      if (roadmapChartInstance.current) roadmapChartInstance.current.destroy();
      if (modalProgressChartInstance.current) modalProgressChartInstance.current.destroy();
      if (modalSkillsGapChartInstance.current) modalSkillsGapChartInstance.current.destroy();
      if (modalRoadmapChartInstance.current) modalRoadmapChartInstance.current.destroy();
    };
  }, []);
  ...
  const loadStudentsData = async () => {
    try {
      ...
      const roadmapSnap = await getDocs(collection(db, "roadmapProgress"));
      ...
      setTimeout(() => createCharts(studentsData), 100);
